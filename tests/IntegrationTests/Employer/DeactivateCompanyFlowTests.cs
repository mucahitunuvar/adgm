...
public class DeactivateCompanyFlowTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public DeactivateCompanyFlowTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    private async Task ApproveCompanyAsync(Guid companyId, string adminAccessToken)
    {
    ...
    [Fact]
    public async Task Deactivate_AsAdmin_OnApprovedCompany_TransitionsToDeactivated()
    {
        var companyId = await RegisterCompanyAsync();
        var accessToken = await LoginAsAdminAsync();
        await ApproveCompanyAsync(companyId, accessToken);

        var request = new HttpRequestMessage(HttpMethod.Post, $"/api/v1/admin/companies/{companyId}/deactivate");
    ...
    [Fact]
    public async Task Deactivate_OnPendingApprovalCompany_ReturnsConflict()
    {
        var companyId = await RegisterCompanyAsync();
        var accessToken = await LoginAsAdminAsync();
