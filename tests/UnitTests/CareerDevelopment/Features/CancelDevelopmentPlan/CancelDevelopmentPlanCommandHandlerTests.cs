...
namespace GenclikMerkezi.UnitTests.CareerDevelopment.Features.CancelDevelopmentPlan;

public class CancelDevelopmentPlanCommandHandlerTests
{
    ...
    private CancelDevelopmentPlanCommandHandler CreateHandler() =>
    ...
    [Fact]
    public async Task Handle_AsCurrentAdvisor_CancelsPlan_AndSaves()
    {
        ...
        var result = await CreateHandler().Handle(new CancelDevelopmentPlanCommand(plan.Id), CancellationToken.None);
    ...
    {
        var result = await CreateHandler().Handle(new CancelDevelopmentPlanCommand(Guid.NewGuid()), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WithAlreadyCancelledPlan_ReturnsConflict()
    {
        var advisorId = Guid.NewGuid();
        var candidateCvId = Guid.NewGuid();
        _careerAdvisorModuleContract.AdvisorIdForCurrentUser = advisorId;
