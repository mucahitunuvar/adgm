...
namespace GenclikMerkezi.UnitTests.Employer.Features.ApproveJob;

public class ApproveJobCommandHandlerTests
{
    ...
    private ApproveJobCommandHandler CreateHandler() =>
    ...
    [Fact]
    public async Task Handle_AsAssignedAdvisor_ApprovesJob_AndNotifiesCompany()
    {
        ...
        var result = await CreateHandler().Handle(new ApproveJobCommand(job.Id), CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal(JobStatus.Published, job.Status);
        Assert.Equal(careerAdvisorId, job.ReviewedByAdvisorId);
        ...
        Assert.Equal(company.ContactEmail, notification.RecipientEmail);
    }
    ...
    {
        var result = await CreateHandler().Handle(new ApproveJobCommand(Guid.NewGuid()), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WithJobNotUnderReview_ReturnsConflict()
    {
        var careerAdvisorId = Guid.NewGuid();
        _careerAdvisorModuleContract.AdvisorIdForCurrentUser = careerAdvisorId;
        var (_, job) = CreateUnderReviewJobForAdvisor(careerAdvisorId);
        job.Approve(careerAdvisorId, DateTime.UtcNow);
