...
public class EmailVerificationTokenTests
{
    ...
    {
        var token = CreateUser().IssueEmailVerificationToken("hash", DateTime.UtcNow.AddHours(48));
    ...
    {
        var token = CreateUser().IssueEmailVerificationToken("hash", DateTime.UtcNow.AddSeconds(-1));
