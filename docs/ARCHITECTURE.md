...
Not: Yukarıdaki taslak `docs/DECISIONS/ADR-024-Website-Module-Design.md` ile ayrıntılandırılıp kabul edilmiştir; modülün tam tasarımı (çok dillilik, içerik çekirdeği, medya kütüphanesi, site ayarları, form motoru, etkinlik kaydı, arama, SEO, güvenlik) için o ADR tek kaynaktır. ADR-024'ün Faz 0'ı (`WebsiteDbContext`, `SiteLanguage`, `MediaAsset` + SkiaSharp tabanlı görsel işleme, `SiteSettings`, isimli policy'ler, `IWebsiteEmailSender` ve `IBotProtectionVerifier` port'ları, `Slug`/`SeoMetadata` value object'leri ve `IHtmlContentSanitizer`/`HtmlSanitizerContentSanitizer` XSS-güvenli HTML sanitizer'ı) tamamlanmıştır; içerik çekirdeği ve sonrası (Faz 1-5) henüz uygulanmamıştır. HTML sanitizer'ın beyaz liste politikası için bkz. SECURITY.md.

Website, ADR-024 §1'de tanımlanan **port** desenini kullanan ilk modüldür (bkz. §50.1): projeye özgü bağımlılıklar modülün kendi kodunda kurulmaz, yalnızca portun implementasyonu kurulur - bu, modülün başka bir projede kod değişikliği olmadan yeniden kullanılabilmesini sağlar. İmplementasyonun nerede yaşadığı neye bağımlı olduğuna göre değişir: e-posta gönderimi başka bir iş modülüne (Notification) dokunduğu için Host composition root'ta bir adaptörle (`NotificationWebsiteEmailSender`) kurulurken, bot doğrulama yalnızca harici bir servise (Cloudflare Turnstile) dokunduğu için doğrudan Website'in kendi Infrastructure'ında (`TurnstileBotProtectionVerifier`) yaşar ve `AddWebsiteModule` tarafından kaydedilir.
...
Website gibi **başka projelerde de kullanılabilir olması gereken** bir modül için bu yeterli değildir: modülün kendisi hiçbir iş moduluna (ör. Notification, Employer) bağımlı olmamalıdır, çünkü o modüller her projede bulunmayabilir. Böyle bir modül kendi **port** arayüzünü tanımlar; portun implementasyonu nerede yaşayacağı, o implementasyonun **başka bir iş moduluna dokunup dokunmadığına** göre değişir:

- **Başka bir iş modülüne dokunuyorsa** (ör. e-posta göndermek için Notification'ı çağırmak), port `GenclikMerkezi.Contracts.<Modül>` altında tanımlanır ve modül onu **implemente etmez** - implementasyon yalnızca Host composition root'ta, o projeye özgü bir adaptör sınıfıyla yapılır (ör. `IWebsiteEmailSender` → Host'taki `NotificationWebsiteEmailSender → INotificationModuleContract`). Başka bir projede yalnızca bu adaptör değişir; modülün kendi kodu hiç değişmez.
- **Yalnızca harici bir servise (üçüncü taraf API, kütüphane) dokunuyorsa, hiçbir iş modülüne değil**, port modülün kendi `Application.Abstractions`'ında tanımlanır ve implementasyonu modülün kendi Infrastructure'ında yaşar, tıpkı `IImageProcessor`/`SkiaSharpImageProcessor` gibi - Host adaptörüne gerek yoktur, çünkü harici bir servise bağımlılık modülün taşınabilirliğini bozmaz (ör. `IBotProtectionVerifier` → Website Infrastructure'daki `TurnstileBotProtectionVerifier`, doğrudan `AddWebsiteModule` tarafından kaydedilir). Başka bir projede yalnızca bu Infrastructure sınıfı değişir/değiştirilir.

Bkz. `docs/DECISIONS/ADR-024-Website-Module-Design.md` §1 (`IWebsiteEmailSender`, `IBotProtectionVerifier`, `IExternalSearchSource` port'ları ve implementasyonlarının nerede yaşadığı).
